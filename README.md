...
- Sends every unresolved email action to **Email Task Review** with **Today**, **Schedule**, **Waiting**, **Completed**, **Edit**, and persistent **Ignore** controls.
- Keeps waiting work in one compact panel; a new incoming reply automatically returns the original task to the next available work block with a visible `↩` marker.
- Lets related email suggestions be selected and converted into one scheduled, editable task.
...
- Reviewed or activated tasks are preserved until Leslee completes or ignores them; ignored Gmail threads do not return after reclassification.
